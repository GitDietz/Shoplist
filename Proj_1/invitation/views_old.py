...
from Proj_1.invitation.models import InvitationKey
from .forms import InvitationKeyForm, InvitationAcceptForm
...
#             # now to hand off to another or process tje key
#         else:
#             template_name = 'invitation/wrong_invitation_key.html'
#
#         extra_context = extra_context is not None and extra_context.copy() or {}
#         extra_context.update({'invitation_key': key})
#         # from here I need a form to get the first/last name and password to create a user
#         # can get the email from the key also the group to join
#         # steps get form data, create username, user, authenticate and log in
#         return None
#     else:
#         return HttpResponseRedirect(reverse('registration_register'))
#
#
# def invite_test(request, key):
#     """
#     23/3/20 here to tes the arrival of ainvited user not registered before.
#     will be processing the url data to complete the registration but not with email confirmation
#     since that is done
#     :param request:
#     :return:
#     """
#     form = InvitationAcceptForm(request.user, request.POST or None)
